...
namespace GameOfDeath
{
	...
	{
		public Scoreboard(Content content, ScreenSpace screen)
		...
		private new readonly ScreenSpace screen;
		...
		internal void DrawDigit(Point position, int digit)
		{
		...
		private new VertexPositionColorTextured GetVertex(Point position, Point uv)
		{
		...
		private void DrawDigits(int prefixDigit, Point position, int dollars)
		{
			...
			for (int index = 0; index < digits.Count; index++)
				DrawDigit(position + DigitOffset(digits.Count, index), digits[index]);
		}
