...
namespace Blocks
{
	...
	public class Game
	{
		...
		protected Constants.DisplayMode DisplayMode { get; set; }
		...
		private void SetKeyboardEvents()
		{
			...
			input.Add(Key.CursorLeft, State.Releasing, () => { Controller.isBlockMovingLeft = false; });
			input.Add(Key.CursorRight, State.Pressing, StartMovingBlockRight);
			input.Add(Key.CursorRight, State.Releasing, () => { Controller.isBlockMovingRight = false; });
