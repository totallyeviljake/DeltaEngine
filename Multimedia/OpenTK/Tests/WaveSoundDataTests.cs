...
namespace DeltaEngine.Multimedia.OpenTK.Tests
{
	public class WaveSoundDataTests
	{
		...
		private static Stream GenerateWrongFormatTestData()
		{
			var stream = new MemoryStream();
			var writer = new BinaryWriter(stream);
			writer.Write(new[] { 'R', 'I', 'F', 'F' });
		...
		private static BinaryWriter GenerateDefaultWaveData()
		{
			var stream = new MemoryStream();
			var writer = new BinaryWriter(stream);
			writer.Write(new[] { 'R', 'I', 'F', 'F' });
		...
		private static void WriteDefaultWaveHeaderChunk(BinaryWriter writer)
		{
			writer.Write(new[] { 'f', 'm', 't', ' ' });
			writer.Write(16);
		...
		private static BinaryReader GenerateMsadpcmTestData()
		{
			var stream = new MemoryStream();
			var writer = new BinaryWriter(stream);
			writer.Write(new[] { 'R', 'I', 'F', 'F' });
		...
		private static void WriteMsadpcmHeaderChunk(BinaryWriter writer)
		{
			writer.Write(new[] { 'f', 'm', 't', ' ' });
			writer.Write(24);
		...
		private static BinaryReader GenerateIeeeTestData()
		{
			var stream = new MemoryStream();
			var writer = new BinaryWriter(stream);
			writer.Write(new[] { 'R', 'I', 'F', 'F' });
		...
		private static void WriteIeeeHeaderChunk(BinaryWriter writer)
		{
			writer.Write(new[] { 'f', 'm', 't', ' ' });
			writer.Write(16);
