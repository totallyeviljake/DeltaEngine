...
namespace DeltaEngine.Multimedia.GLFW
{
	public sealed class GLFWSoundDevice : SoundDevice
	{
			...
			sampleRate)
		{
			...
			try
			{
				AL.BufferData((uint)bufferHandle, AudioFormatToALFormat(format), 
					gcHandle.AddrOfPinnedObject(), length, sampleRate);
			} finally
			{
				gcHandle.Free();
			}
		...
		{
			AL.SourceQueueBuffers((uint)channelHandle, 1, new[] {
				(uint)bufferHandle
			});
		}
		...
		public int GetNumberOfBuffersQueued(int channelHandle)
		{
			...
			AL.GetSource((uint)channelHandle, (ALSourcei)ALGetSourcei.BuffersQueued, out 
				numberOfBuffersQueued);
		...
		public int GetNumberOfBuffersProcesed(int channelHandle)
		{
			...
			AL.GetSource((uint)channelHandle, (ALSourcei)ALGetSourcei.BuffersProcessed, out 
				numberOfBufferProcessed);
