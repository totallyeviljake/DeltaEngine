...
{
	public class WindowTests : TestWithAllFrameworks
	{
		[VisualTest]
		public void CreateWindow(Type resolver)
		{
			Start(resolver, (Window window) => Assert.IsTrue(window.Visibility));
		}
		...
		public void ChangeTotalSize(Type resolver)
		{
			Start(resolver, (Window window) =>
			{
				Assert.AreEqual(new Size(640, 360), window.TotalPixelSize);
